# Testing Guide for NetBox Device Patterns Plugin

This guide explains how to write and run tests for NetBox Device Patterns Plugin.

## Test Structure

Tests are organized in the `netbox_device_patterns_plugin/tests/` directory:
...
```python
from netbox_device_patterns_plugin.testing import PluginTestCase
...
```python
from netbox_device_patterns_plugin.testing import PluginModelTestCase
...
```python
from netbox_device_patterns_plugin.testing import PluginAPITestCase

class MyAPITestCase(PluginAPITestCase):
    def setUp(self):
        super().setUp()
        self.list_url_name = 'plugins-api:netbox_device_patterns_plugin-api:mymodel-list'
        self.detail_url_name = 'plugins-api:netbox_device_patterns_plugin-api:mymodel-detail'
...
```python
from netbox_device_patterns_plugin.testing import PluginViewTestCase

class MyViewTestCase(PluginViewTestCase):
    def setUp(self):
        super().setUp()
        self.base_url = 'plugins:netbox_device_patterns_plugin:mymodel'
...
```python
from netbox_device_patterns_plugin.testing import PluginGraphQLTestCase
...
The `netbox_device_patterns_plugin.testing.utils` module provides helpful utilities:

```python
from netbox_device_patterns_plugin.testing.utils import (
...
user = create_test_user(permissions=['netbox_device_patterns_plugin.view_devicepatterns'])
...
python manage.py test netbox_device_patterns_plugin.tests

# Run specific test file
python manage.py test netbox_device_patterns_plugin.tests.test_models

# Run specific test class
python manage.py test netbox_device_patterns_plugin.tests.test_models.DevicepatternsTestCase

# Run specific test method
python manage.py test netbox_device_patterns_plugin.tests.test_models.DevicepatternsTestCase.test_create

# Run with verbose output
python manage.py test netbox_device_patterns_plugin.tests -v 2

# Run in parallel (faster)
python manage.py test netbox_device_patterns_plugin.tests --parallel

# Keep database between runs (faster during development)
python manage.py test netbox_device_patterns_plugin.tests --keepdb
```
...
# Run tests
docker-compose exec netbox python manage.py test netbox_device_patterns_plugin.tests

# Run specific tests
docker-compose exec netbox python manage.py test netbox_device_patterns_plugin.tests.test_api
...
def test_form_validation(self):
    ...
    url = reverse('netbox_device_patterns_plugin:mymodel_add')
...
**"No module named 'netbox_device_patterns_plugin'"**
